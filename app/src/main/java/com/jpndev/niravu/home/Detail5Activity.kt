...
import kotlinx.android.synthetic.main.rcv_item_home_data_5.view.text1_txv

class Detail5Activity : NiravuActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        item?.let {
            ...
            defSetHtmlText(text1_txv, it.detail_text1?.let { it } ?: it.text1?.let { it }?:let { "" })
          ...
          if(it.design_override) {
            ...
            //com.squareup.picasso.Picasso.with(this@Detail5Activity).load(android.text.Html.fromHtml(it.image1!!).toString()).into(banner_imv)  ;

            Picasso.with(this@Detail5Activity).load(Html.fromHtml(it.image1!!).toString()).placeholder(R.drawable.placeholder_image).error(R.drawable.placeholder_image).into(object :
                Target {
                override fun onBitmapLoaded(bitmap: Bitmap, from: Picasso.LoadedFrom) {

                    banner_imv.setImageBitmap(bitmap)

                    banner_imv.setOnClickListener(View.OnClickListener {


                        ImageZoomDialogFragment.showImageDialog(this@Detail5Activity, bitmap, true)
    ...
    private fun setValues() {
        ...
        appViewModel.appRepository.s_pref.stringLiveData("app_object", "").observe(this,
            Observer { json ->
                ...
                LogUtils.LOGD("niravu", "Detail5Activity   app_object json=  "+json)
                if(isValid(json)){
    ...
    private fun setBackGrounds(   app: MApp?) {
        ...
        app?.let {
            ...
            it.bg_detail_1?.let {
                ...
                LogUtils.LOGD("niravu", "Detail5Activity app_object bg_splash_1=  "+it)
                Picasso.with(this@Detail5Activity).load(Html.fromHtml(it!!).toString()).placeholder(R.drawable.icn_niravu_bg6_2).error(R.drawable.icn_niravu_bg6_2).into(object :
                    Target {
                    override fun onBitmapLoaded(bitmap: Bitmap, from: Picasso.LoadedFrom) {
                ...
                //com.squareup.picasso.Picasso.with(this@Detail5Activity).load(android.text.Html.fromHtml(it).toString()).into(top_clay)  ;
            }
