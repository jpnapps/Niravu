...
open class Chameleon(context: Context?, attrs: AttributeSet?) : ConstraintLayout(context, attrs) {
    ...
    private var stateSpaceTopTextView: AppCompatTextView? = null
            ...
            a?.let {
                chameleonAttr =
                        ChameleonAttr(
                                ...
                                it.getResourceId(R.styleable.Chameleon_buttonDrawable, R.drawable.ic_chameleon_error).getDrawable(context),
                                it.getBoolean(R.styleable.Chameleon_useButtonDrawable, false),
                                stateFromInt(it.getInt(R.styleable.Chameleon_dState, -1))
                        )
            }
     ...
     * If the `dState` is not set by the developer,
    ...
    private fun initSpaceTopTextView() {
        val padding = 60.DP(context).toInt()
        stateSpaceTopTextView = AppCompatTextView(context)
        stateSpaceTopTextView?.apply {
            id = R.id.tv_space_top_state
            text="dddddd"
        ...
        super.addView(stateSpaceTopTextView, layoutParams)
    }
        ...
        when (state) {
            ...
            STATE.ERROR -> {
                chameleonAttr?.let {
                 ...
                 /*   setStateSpaceTopTextView(titleTextSettingBundle.text ?: it.emptyText,
                            100.0f,
...
*/
                    ...
                    if (it.useErrorButton)
                        setStateButton(buttonSettingBundle.text ?: it.errorButtonText,
                                ...
                                buttonSettingBundle.buttonDrawable
                                        ?: it.buttonDrawable,
                                buttonSettingBundle.useButtonDrawable
                                        ?: it.useButtonDrawable,
    ...
    private fun setViewVisibility(contentViewVisible: Int = View.GONE,
       ...
       // stateSpaceTopTextView?.visibility =  View.VISIBLE
    ...
    private fun setStateSpaceTopTextView(content: String, size: Float, color: Int, gravity: Int) {
   /*     stateSpaceTopTextView?.apply {
            this.gravity = gravity
            text = content
            setTextSize(TypedValue.COMPLEX_UNIT_PX, size)
    ...
    private fun setStateButton(content: String,
                               ...
                               useBackgroundDrawable: Boolean=false,
                               listener: ((View) -> Unit)?) {
        stateButton?.apply {
            text = content
            setTextSize(TypedValue.COMPLEX_UNIT_PX, size)
    ...
    private fun setStateButtonDrawable(content: String,
                               size: Float,
                               textColor: Int,
                               backgroundColor: Int,
